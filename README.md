...
L'interfaccia usa quattro tab native: **Inizio**, **Studio**, **Risultati** e **Voce**. Puoi scegliere audio e video da **File**, inclusi gli MP4 etichettati diversamente dai provider, oppure un video dalla **galleria Foto**. Per diagnosticare un errore, abilita **Debug** in Impostazioni iOS → NeMo Studio; trovi `session.log` in **File → Sul mio iPhone → NeMo Studio** e puoi esportarlo da lì. Con Debug attivo il file viene sovrascritto al nuovo avvio. Sono disponibili anche **«Video + traccia»** (MKV con sottotitoli selezionabili per H.264/HEVC + AAC) e **«Tutto»**, che produce le uscite video compatibili. La compilazione e l'IPA sono verificate in CI; inferenza e riproduzione dei risultati richiedono una prova su iPhone.
